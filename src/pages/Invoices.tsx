      ...
      {selectedInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs print:static print:inset-auto print:bg-transparent print:p-0 print:flex-none">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto border border-slate-100 animate-in fade-in zoom-in-95 duration-150 print:shadow-none print:border-none print:max-w-none print:max-h-none print:overflow-visible print:animate-none">
            ...
            <div className="p-8 print:p-8 print:w-[210mm] print:mx-auto space-y-6 print:space-y-8 text-xs print:text-sm text-slate-700 bg-white">
              {/* Top Banner */}
              <div className="flex justify-between items-start border-b border-slate-100 pb-6 print:pb-6">
                <div className="flex items-center gap-3 print:gap-4">
                  {clinicProfile.logo ? (
                    <img 
                      src={clinicProfile.logo} 
                      alt="Logo" 
                      className="w-10 h-10 print:w-16 print:h-16 object-contain rounded-xl p-1 bg-slate-50 border border-slate-100"
                    />
                  ) : (
                    <div className="w-10 h-10 print:w-16 print:h-16 rounded-xl bg-[#2563eb] text-white flex items-center justify-center shadow-xs">
                      <svg className="w-6 h-6 print:w-10 print:h-10 fill-current" viewBox="0 0 24 24">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 text-sm print:text-xl">{clinicProfile.name || 'Bright Smile Dental Clinic'}</h3>
                    <p className="text-slate-400 text-[11px] print:text-xs">{clinicProfile.phone || '+233 30 274 1122'}</p>
                    <p className="text-slate-400 text-[11px] print:text-xs">{clinicProfile.email || 'hello@brightsmiledental.com'}</p>
                  </div>
                </div>

                <div className="text-right">
                  <span className="text-[10px] print:text-xs font-bold text-slate-400 uppercase tracking-widest block">
                    {(selectedInvoice.type || 'INVOICE').toUpperCase()}
                  </span>
                  <span className="font-bold font-mono text-slate-900 text-sm print:text-base block mt-0.5">{selectedInvoice.invoiceNumber}</span>
                  <span className="text-slate-400 text-[11px] print:text-xs block mt-0.5">{selectedInvoice.invoiceDate || selectedInvoice.date}</span>
                </div>
              </div>

              {/* Billed To & Status */}
              <div className="grid grid-cols-2 gap-4 pb-4 print:pb-6 border-b border-slate-100">
                <div>
                  <p className="text-[10px] print:text-xs font-bold text-slate-400 uppercase">
                    {selectedInvoice.type === 'Quotation' ? 'PREPARED FOR' : 'BILLED TO'}
                  </p>
                  <p className="font-bold text-slate-900 text-xs print:text-base mt-1">{selectedInvoice.patientName}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] print:text-xs font-bold text-slate-400 uppercase">
                    {selectedInvoice.type === 'Quotation' ? 'VALID UNTIL' : 'DUE DATE'}
                  </p>
                  <p className="font-medium text-slate-800 text-xs print:text-base mt-1">{selectedInvoice.dueDate}</p>
                </div>
              </div>
              ...
              <table className="w-full text-xs print:text-sm text-left">
                <thead>
                  <tr className="text-slate-400 font-medium border-b border-slate-100 print:border-slate-200 text-slate-500">
                    <th className="pb-2 print:pb-3">Service</th>
                    <th className="pb-2 print:pb-3 text-center">Qty</th>
                    <th className="pb-2 print:pb-3 text-right">Price</th>
                    <th className="pb-2 print:pb-3 text-right">Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 print:divide-slate-100">
                  {(selectedInvoice.items || []).map((item: any, idx: number) => (
                    <tr key={idx}>
                      <td className="py-2.5 print:py-3 font-medium text-slate-800">{item.serviceName}</td>
                      <td className="py-2.5 print:py-3 text-center text-slate-600">{item.quantity}</td>
                      <td className="py-2.5 print:py-3 text-right text-slate-600">GH₵ {Number(item.unitPrice).toFixed(2)}</td>
                      <td className="py-2.5 print:py-3 text-right font-bold text-slate-900">GH₵ {Number(item.total).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {/* Totals */}
              <div className="pt-4 print:pt-6 border-t border-slate-100 print:border-slate-200 flex justify-end">
                <div className="w-56 print:w-64 space-y-1.5 print:space-y-2 text-xs print:text-sm">
                  <div className="flex justify-between text-slate-500">
                    <span>Subtotal</span>
                    <span>GH₵ {Number(selectedInvoice.subtotal || selectedInvoice.total).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between font-bold text-slate-900 pt-1 print:pt-2 border-t border-slate-100">
                  ...
                  {selectedInvoice.type !== 'Quotation' && (
                    <>
                      ...
                      <div className="flex justify-between font-bold text-red-500 pt-1 print:pt-2 border-t border-slate-100 text-sm print:text-base">
